/* eslint-env browser */
import { loadBitmapByFile, loadBitmapByUrl, loadSplatBitmapByFile, loadSplatBitmapByUrl, loadRadBitmapByFile, loadRadBitmapByUrl, } from './lib/bitmap-loader';
...
function main() {
    ...
    biomesInput.addEventListener('input', async () => {
        ...
        const biomesImg = await loadBitmapByFile(window, (biomesInput as any).files[0]);
    ...
    splat4Input.addEventListener('input', async () => {
        ...
        const splat4Img = await loadSplatBitmapByFile(window, (splat4Input as any).files[0]);
    ...
    radInput.addEventListener('input', async () => {
        ...
        const radImg = await loadRadBitmapByFile(window, (radInput as any).files[0]);
    ...
    prefabsInput.addEventListener('input', async () => {
        ...
        const prefabs = await loadPrefabsXmlByFile(window, (prefabsInput as any).files[0]);
    ...
    ['input', 'focus'].forEach((eventName) => {
        ...
        prefabsFilterInput.addEventListener(eventName, async () => {
            prefabsFilterWorker.postMessage({ prefabsFilterString: (prefabsFilterInput as any).value });
        });
        // @ts-expect-error ts-migrate(2531) FIXME: Object is possibly 'null'.
        blocksFilterInput.addEventListener(eventName, async () => {
            prefabsFilterWorker.postMessage({ blocksFilterString: (blocksFilterInput as any).value });
        });
    });
    const restInputs = [
    ...
    async function sequential(promiseGenerators: any) {
        await promiseGenerators.reduce((prevPromise: any, generator: any) => prevPromise.then(() => generator()), Promise.resolve());
    }
    ...
    mapCanvas.addEventListener('click', async (event) => {
        ...
        const markCoords = {
            x: Math.round((event.offsetX * mapSizes.width) / (mapCanvas as any).width - mapSizes.width / 2),
            z: -Math.round((event.offsetY * mapSizes.height) / (mapCanvas as any).height - mapSizes.height / 2),
        };
    ...
    (new MutationObserver((mutationsList) => {
        ...
        if (!heightMutation)
            return;
        const newCanvasSize = { width: (mapCanvas as any).width, height: (mapCanvas as any).height };
        if (!markPosition) {
    ...
    Array.from(document.querySelectorAll('[data-source-input')).forEach((display) => {
        const sourceInput = document.querySelector(`#${(display as any).dataset.sourceInput}`);
        display.textContent = (sourceInput as any).value;
        // @ts-expect-error ts-migrate(2531) FIXME: Object is possibly 'null'.
        sourceInput.addEventListener('input', () => { display.textContent = (sourceInput as any).value; });
    });
