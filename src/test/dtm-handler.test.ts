...
describe("Dtm.writeZ", () => {
  it("fills z from DTM data for each vertex", () => {
    ...
    for (let i = 0, j = 0; i < pos.count; i++, j += 3) {
      ...
      expect(arr[j + 2]).toBe(expected);
      expect(pos.getZ(i)).toBe(expected);
    }
  });
  ...
  it("clamps out-of-range coordinates to the DTM boundary", () => {
    ...
    arr[1] = 100; // y -> game coords far beyond +halfH
    ...
    expect(arr[2]).toBeCloseTo(expected, 5);
  });
  ...
  it("throws when position attribute itemSize is not 3", () => {
    ...
    expect(() => dtm.writeZ(geo)).toThrow("Unexpected item size");
  });
});
