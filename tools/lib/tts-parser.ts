...
export async function parseTts(ttsFileName: any): Promise<any> {
  ...
  function handleByte(byte: any) {
    ...
    if (skipBytes !== 0) {
      ...
      (dimensions as any).x = Buffer.from([
        (dimensions as any).xSecondDigit,
        byte,
      ]).readInt16LE();
      ...
      (dimensions as any).y = Buffer.from([
        (dimensions as any).ySecondDigit,
        byte,
      ]).readInt16LE();
      ...
      (dimensions as any).z = Buffer.from([
        (dimensions as any).zSecondDigit,
        byte,
      ]).readInt16LE();
      blockIdsNum =
        (dimensions as any).x * (dimensions as any).y * (dimensions as any).z;
      ...
      const bid =
        Buffer.from(digits.slice(0, 2)).readInt16LE() & 0b0011111111111111;
